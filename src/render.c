...
const struct {
  ...
  struct {
    ...
    {{BORDER_STRIP_WIDTH, CENTER_Y - GOAL_AREA_WIDTH / 2.0f},
     {BORDER_STRIP_WIDTH + FIELD_LENGTH - GOAL_AREA_LENGTH,
      CENTER_Y - GOAL_AREA_WIDTH / 2.0f},
     {GOAL_AREA_LENGTH, GOAL_AREA_WIDTH}},
    {{BORDER_STRIP_WIDTH, CENTER_Y - PENALTY_AREA_WIDTH / 2.0f},
     {BORDER_STRIP_WIDTH + FIELD_LENGTH - PENALTY_AREA_LENGTH,
      CENTER_Y - PENALTY_AREA_WIDTH / 2.0f},
...
int Render_Create(struct Render *render, unsigned int width,
  ...
  render->window = glfwCreateWindow(width * scale, height * scale,
                                    "Tactics Sim", NULL, NULL);
  if (!render->window)
    return 2;
  ...
  render->pcmx = (float)render->width / width,
  render->pcmy = (float)render->height / height;
...
void _render_vertex(struct Render render, vec2_t p) {
  glVertex2f(p.x / render.width * render.pcmx * 2 - 1,
             p.y / render.height * render.pcmy * -2 + 1);
}
...
void drawField(struct Render render) {
  ...
  // Center Line
  Render_Line(render, (vec2_t){CENTER_X, BORDER_STRIP_WIDTH},
              (vec2_t){BORDER_STRIP_WIDTH + FIELD_LENGTH / 2.0f,
                       BORDER_STRIP_WIDTH + FIELD_WIDTH});

  // Center Circle
  Render_PolyLine(render, 8,
                  (vec2_t){CENTER_X, CENTER_Y - CENTER_CIRCLE_DMTR / 2.0f},
                  (vec2_t){CENTER_X + CENTER_CIRCLE_DMTR / 2.0f, CENTER_Y},
                  (vec2_t){CENTER_X + CENTER_CIRCLE_DMTR / 2.0f, CENTER_Y},
                  (vec2_t){CENTER_X, CENTER_Y + CENTER_CIRCLE_DMTR / 2.0f},
                  (vec2_t){CENTER_X, CENTER_Y + CENTER_CIRCLE_DMTR / 2.0f},
                  (vec2_t){CENTER_X - CENTER_CIRCLE_DMTR / 2.0f, CENTER_Y},
  ...
  // center cross
  Render_Line(render, (vec2_t){CENTER_X - 10, CENTER_Y},
              (vec2_t){CENTER_X + 10, CENTER_Y});

  // crosses
  Render_Line(
      render,
      (vec2_t){BORDER_STRIP_WIDTH + PENALTY_MARK_DIST - CROSS_SIZE / 2.0f,
               CENTER_Y},
      (vec2_t){BORDER_STRIP_WIDTH + PENALTY_MARK_DIST + CROSS_SIZE / 2.0f,
               CENTER_Y});
  Render_Line(render,
              (vec2_t){BORDER_STRIP_WIDTH + PENALTY_MARK_DIST,
                       CENTER_Y - CROSS_SIZE / 2.0f},
              (vec2_t){BORDER_STRIP_WIDTH + PENALTY_MARK_DIST,
                       CENTER_Y + CROSS_SIZE / 2.0f});
  Render_Line(render,
              (vec2_t){BORDER_STRIP_WIDTH + FIELD_LENGTH - PENALTY_MARK_DIST -
                           CROSS_SIZE / 2.0f,
                       CENTER_Y},
              (vec2_t){BORDER_STRIP_WIDTH + FIELD_LENGTH - PENALTY_MARK_DIST +
                           CROSS_SIZE / 2.0f,
  ...
  Render_Rect(render, field.penalty_area.l,
              field.penalty_area.size); // penalty areas
...
void drawMarker(struct Render render, body_t body, int fallen, float r, float g,
  ...
  if (fallen) {
    ...
    Render_Line(
        render,
        (vec2_t){body.center.x - MARKER_RADIUS, body.center.y + MARKER_RADIUS},
        (vec2_t){body.center.x - MARKER_RADIUS, body.center.y - MARKER_RADIUS});
